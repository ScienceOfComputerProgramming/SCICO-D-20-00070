...
The first major challenge concerns Git as a lightweight system that involves single threaded file access operations. When building the issue repository from past commits, SCIIT traverses each file object scans its contents to determine if an issue is present or not. This is a time-consuming process and is restricted to a single thread. With multiple threads, the system  encounters I/O errors, especially since there is a high likelihood that the same file may be accessed at the same time.
